...
describe('Observer problem', () => {
  it('should be buy a shoe', () => {
    ...
    expect(c1.buyShoes('Nike')).toBe('John bought Nike')
    expect(c2.buyShoes('Adidas')).toBe('Jane bought Adidas')
    expect(c3.buyShoes('Puma')).toBe('Jack bought Puma')
  })
  it("should not be buy a shoe if store have't shock", () => {
    const store = new ShoesStore()
    const c1 = new Customer('John', store)
    expect(c1.buyShoes('Nike')).toBe('Shoe not in stock')
  })
